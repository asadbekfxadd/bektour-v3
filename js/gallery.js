...
/* BEK TOUR - auto gallery (photo + video)
   Folder files: 1.jpg, 2.mp4, 3.webp...
   Hero: images/hero/1.webp = slide 1 bg, 2.jpg = slide 2... */
...
async function initGallery(el) {
  ...
  el.innerHTML = items.map((item, idx) => {
    ...
    return `<div class="gallery-item" onclick="openLightbox('${item.url}')">
...
/* Hero backgrounds: images/hero/1.webp -> slide 1, 2.jpg -> slide 2, 3.jpg -> slide 3 */
async function initHeroBackgrounds() {
  ...
  for (let i = 0; i < slides.length; i++) {
    ...
    if (url) slides[i].style.backgroundImage = `url('${url}')`;
  }
}
